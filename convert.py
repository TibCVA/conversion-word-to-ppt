...
Structure attendue du Word :
  ...
  - Ensuite, une ligne "Titre :" suivie du titre
  - Puis une ligne "Sous-titre / Message clé :" suivie du sous-titre
  - Puis le reste du contenu (paragraphes, listes, …)

Le template PPT doit comporter deux layouts :
  • "Diapositive de titre" : pour la slide de couverture  
      -> il doit contenir des zones de texte dont le type de placeholder est TITLE et SUBTITLE.
  • "Slide_standard layout" : pour les slides standard  
      -> il doit contenir des zones de texte de type TITLE, SUBTITLE et BODY.
      
Usage :
  python convert.py input.docx output.pptx
Le template est supposé être nommé "template_CVA.pptx" et situé dans le même dossier que ce script.
"""
...
# --------------------------------------------------------------------
# Parcours récursif des shapes (pour gérer les group-shapes)
# --------------------------------------------------------------------
def iterate_all_shapes(shape_collection):
    """
    Génère chaque shape de shape_collection, et si une shape est un group,
    génère également récursivement ses sous-shapes.
    """
...
def parse_docx_to_slides(doc_path):
    """
    Retourne une liste de dictionnaires, chacun correspondant à une slide.
    Chaque dictionnaire contient : {"title": str, "subtitle": str, "blocks": [...]}
    Les blocks sont des tuples ("paragraph", paragraph_obj).
    """
...
# --------------------------------------------------------------------
# Gestion des listes (détection bullet/decimal) et insertion de runs
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Remplir les zones de texte de la slide en fonction du type de placeholder
# --------------------------------------------------------------------
def fill_shapes_text(slide, slide_data, slide_index):
    if slide_index == 0:
        # Pour la slide de couverture : on remplit TITLE et SUBTITLE
        for shp in iterate_all_shapes(slide.shapes):
            ...
            if shp.placeholder_format is not None:
                ...
                # Fallback si la shape n'est pas reconnue comme placeholder
        ...
        # Pour les slides standards : TITLE, SUBTITLE et BODY
        for shp in iterate_all_shapes(slide.shapes):
            if not shp.has_text_frame:
                continue
            if shp.placeholder_format is not None:
                ...
                # Fallback via recherche de texte
                txt_ref = shp.text.strip().lower()
...
def create_ppt_from_docx(input_docx, template_pptx, output_pptx):
    ...
    # Rechercher les layouts par leur nom
    cover_layout = None
